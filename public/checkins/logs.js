...
async function getData() {
  ...
  for (item of data) {
    ...
    this ${item.weather.summary} day and it feels like ${
    ...
    if (item.air.value < 0) {
      ...
      txt += `  
      The concentration of small carcinogenic particles (${
        item.air.measurements[0].parameter
      }) I'm
        breathing in is  ${item.air.measurements[0].value} ${
        item.air.measurements[0].unit
      } measured from
       ${item.air.city} at ${item.air.location} on ${
        item.air.measurements[0].lastUpdated
      }.`;
    }
